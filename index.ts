...
const DIRECT_MESSAGES_SECTION_ID = "vc-group-dms-direct-messages";
const GROUP_MESSAGES_SECTION_ID = "vc-group-dms-group-messages";
const PRIVATE_CHANNELS_SELECTOR = ".privateChannels_e6b769";
const COLLAPSED_CLASS = "vc-group-dms-collapsed";
const GROUPS_COLLAPSED_CLASS = "vc-group-dms-groups-collapsed";
...
function syncRootCollapseState() {
  const root = getRoot();
  const directMessagesSection = document.getElementById(DIRECT_MESSAGES_SECTION_ID);
  const groupMessagesSection = document.getElementById(GROUP_MESSAGES_SECTION_ID);

  if (!root || !directMessagesSection || !groupMessagesSection) return;

  root.classList.toggle(COLLAPSED_CLASS, directMessagesSection.classList.contains("collapsed"));
  root.classList.toggle(GROUPS_COLLAPSED_CLASS, groupMessagesSection.classList.contains("collapsed"));
}
...
function ensurePanel() {
  ...
  if (!panel) {
    ...
    directMessagesHeader.className = "vc-group-dms-section-header";
    directMessagesHeader.innerHTML = "<span data-vc-gdm-toggle-icon>▸</span><span>Direct Messages</span>";

    const directMessagesSection = document.createElement("div");
    directMessagesSection.id = DIRECT_MESSAGES_SECTION_ID;
    directMessagesSection.className = "vc-group-dms-section-body collapsed";
    ...
    directMessagesHeader.addEventListener("click", () => {
      directMessagesSection.classList.toggle("collapsed");
      directMessagesHeader.classList.toggle("collapsed", directMessagesSection.classList.contains("collapsed"));
    ...
    panel.append(directMessagesHeader, directMessagesSection, separator, groupMessagesHeader, groupMessagesSection);
...
function stopObservers() {
  ...
  getRoot()?.classList.remove(COLLAPSED_CLASS, GROUPS_COLLAPSED_CLASS);
}
